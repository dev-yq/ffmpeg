...
;
    //缓存区
    int numBytes = av_image_get_buffer_size(AV_PIX_FMT_RGBA, ffmpegVideo->codec->width, ffmpegVideo->codec->height, 1);
    uint8_t * v_out_buffer = (uint8_t *)av_malloc(numBytes*sizeof(uint8_t));
    //与缓存区相关联，设置rgb_frame缓存区

    av_image_fill_arrays(rgb_frame->data, rgb_frame->linesize, v_out_buffer, AV_PIX_FMT_RGBA,  ffmpegVideo->codec->width, ffmpegVideo->codec->height, 1);
